        ...
        try {
            ...
            for (Object o : individualInfo) {
                ...
                if ("法人".equals(leadType)) {
                    fr_leadName = jsonObject.getString("leadName");
                    fr_certificateNum = jsonObject.getString("certificateNum");
                    fr_leadPhone = jsonObject.getString("leadPhone");
                    fr_realNameStatus = jsonObject.getString("realNameStatus");
                    if ("1".equals(fr_realNameStatus)) {
                        fr_realNameStatus = "已实名";
                    } else {
                        fr_realNameStatus = "未实名";
                    }
                    fr_individualProvince = jsonObject.getString("individualProvince");
                    ...
                    if (dataDictionaryList.size() > 0) {
                        fr_individualProvince = dataDictionaryList.get(0).get("zdz") == null ? "" : dataDictionaryList.get(0).get("zdz") + "";
                    }
                    fr_individualCity = jsonObject.getString("individualCity");
                    List<CCObject> city_list = cs.cquery("city", "dqbm='" + fr_individualCity + "' and is_deleted='0'");
                    if (city_list.size() > 0) {
                        fr_individualCity = city_list.get(0).get("name") == null ? "" : city_list.get(0).get("name").toString();
                    }
                    fr_individualCounty = jsonObject.getString("individualCounty");
                    List<CCObject> country_list = cs.cquery("country", "dqbm='" + fr_individualCounty + "' and is_deleted='0'");
                    if (country_list.size() > 0) {
                        fr_individualCounty = country_list.get(0).get("name") == null ? "" : country_list.get(0).get("name").toString();
                    }
                    fr_individualAddress = jsonObject.getString("individualAddress");
                } else {
...
//                    lead.put("recordtype", "2020F5E1802867Fsfq0B");
                ...
                if (lead_list.size() > 0) {
                    ...
                    for (Object o : files) {
                        ...
                        CCObject gather_data = new CCObject("gather_data");
                    ...
                    if ("".equals(registeredProvince) || "".equals(company) || "".equals(socialCreditCode)) {
                        ...
                        if ("关闭已转换".equals(qzkhzt)) {
                            List<CCObject> account_list = cs.cquery("Account", "sflzqzkh='" + id + "' and is_deleted='0'");
                            String ownerid = "";
                            if (account_list.size() > 0) {
                            ...
                            CCObject ylqyxx = new CCObject("ylqyxx");
...
//                                String noticeUrl = "https://k8mm3cmt3235c7ed72cede6e.cloudcc.com/customize/page/208f9f6b9a/noticeRuleCalculateResult.jsp?name=noticeRuleCalculateResult";//
                                ...
                                JSONObject rule_data = new JSONObject();
                                ...
                                JSONObject rule_data_json = new JSONObject();
